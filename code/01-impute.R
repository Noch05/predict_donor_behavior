...
# Use the appropriate plan for your system
...
data <- data %>%
  mutate(
    donate = factor(donate, levels = c(0, 1), labels = c("No", "Yes")),
...
train$data <- train$data |>
  mutate(
    weights = if_else(
      donate == "Yes",
      nrow(train$data) / (2 * sum(donate == "Yes")),
      nrow(train$data) / (2 * sum(donate == "No"))
    ),

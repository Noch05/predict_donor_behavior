...
files <- c(
  "cces_2006_common.dta", "CCES07_OUTPUT.sav", "cces_2008_common.dta", "CCES_2010_CommonContent.sav",
...
doi <- c(
  "10.7910/DVN/Q8HC9N/OK6YQS", "10.7910/DVN/OOXTJ5/C9WH7I", "10.7910/DVN/YUYIVB/P3MCIW",
...
process <- function(data, year, mapping) {
  ...
  data <- data %>%
    mutate(
      ...
      race = factor(
        recode(!!sym(mapping[[as.character(year)]]$race),
          `1` = "White", `2` = "Black",
          `3` = "Hispanic", `4` = "Asian",
          `5` = "Native American",
          .default = "Other"
      ...
      ideology = factor(
        recode(!!sym(mapping[[as.character(year)]]$ideo5),
          `1` = "Very Liberal",
          `2` = "Liberal",
          `3` = "Moderate",
          `4` = "Conservative",
      ...
      pol_activites = case_when(
        year %in% c(2006, 2007) ~
          if_else(any(c(
            !!sym(mapping[[as.character(year)]]$group_1),
            ...
            !!sym(mapping[[as.character(year)]]$group_12)
          )) == 1, 1, 0),
        year %in% c(2019, 2017) ~
      ...
      immigrant = factor(
        recode(!!sym(mapping[[as.character(year)]]$immstat),
          .default = "Born Citizen",
          `1` = "Immigrant Citizen",
          `2` = "Immigrant Noncitizen"
        ),
        levels = c(
          "Born Citizen", "Immigrant Citizen",
      ...
      religion = case_when(
        year %in% c(2006, 2007) ~
          factor(
            recode(!!sym(mapping[[as.character(year)]]$relig),
              `1` = "Protestant",
              `2` = "Catholic",
              `3` = "Jewish",
              `4` = "Muslim",
            ...
            levels = c(
              "Nothing", "Protestant", "Catholic", "Mormon", "Orthodox",
      ...
      relig_imp = case_when(
        year %in% c(2006, 2007) ~
          factor(
            recode(!!sym(mapping[[as.character(year)]]$religimp),
              "1" = "Very",
              `2` = "None",
              .default = "None"
            ),
      ...
      income = case_when(
        year %in% c(2018, 2022, 2020, 2016, 2014, 2012, 2017, 2019) ~
          factor(
            recode(!!sym(mapping[[as.character(year)]]$inc),
              `1` = "<10k",
              `2` = "10k-20k",
              `3` = "20k-30k",
              `4` = "30k-40k",
            ...
            levels = c(
              "<10k", "10k-20k", "20k-30k", "30k-40k", "40k-50k", "50k-60k", "60k-70k",
    ...
    select(
      year, state, male, race, educ, marriage, ideology, votereg, union,
